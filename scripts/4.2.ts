...
import { ANA_FOLDER_PATH, DAT_FOLDER_PATH } from "./consts.ts";
...
interface SchemaInfo {
  tables: Array<
    { name: string; sql: string; samples: Array<Record<string, unknown>> }
  >;
}
...
for (const dbPath of matchedDbFiles) {
  ...
  try {
    const tableResults = db.query(
      `SELECT name, sql FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'`,
    );
    for (
      const [tableName, createSql] of tableResults as Array<[string, string]>
    ) {
...
if (!queryData.choices || queryData.choices.length === 0) {
  console.error(
    "❌ Albert API error (query generation):",
...
for (const dbPath of matchedDbFiles) {
  ...
  try {
    ...
    console.log(
      `  ✓ Query executed successfully. Got ${queryResults.length} rows.`,
    );
    db.close();
    break;
  } catch (e) {
...
const albertSummaryPrompt =
  `Voici les résultats d'une requête SQL en réponse à la question: "${prompt}"\n\nRésultats:\n${
