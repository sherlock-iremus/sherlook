...
const { options } = await new Command()
  .name("SHERLOOK 3.1.3")
  .description(
    "Aggregate files matching regex and extract structure via Albert",
  )
...
const { repo, prompt, albertBase, albertApiKey, collectionUuid, inputRegex } =
  options;
...
`,
  ...
  if (!res.ok) {
    ...
    console.error(
      `❌ Albert API error (status ${res.status} ${res.statusText}):`,
    );
...
await Deno.writeTextFile(
  ...
  stringify(allResults, {
    header: true,
    columns: Object.keys(allResults[0] || {}),
  }),
);
console.log(
