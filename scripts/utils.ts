...
import {
  addRecords,
  fetchRecords as fetchGristRecords,
  patchRecord as putRecords,
  writeValueById,
} from "https://gitlab.huma-num.fr/sherlock/sherlock-deno/-/raw/main/common-grist.ts";
  ...
  if (ext && ext.toLowerCase() === "pdf") {
    try {
      ...
      pages = data.numpages || data.numpages === 0
        ? data.numpages
        : (data.info && data.info.Pages) || null;
    } catch (e) {
...
export const getRunName = (cliRunName: string, tool: string) => {
  return cliRunName + "__" + tool 
}
...
export const addRunRecordToGrist = async (
  options: any,
  collectionId: string,
  tool: string,
  runName: string,
    ...
    if (existingRunId) {
      await putRecords(
        ...
        options.gristRunTableId,
        {
          records: [{
            fields: runRecord,
            require: { collection: collectionId, name: runName },
          }],
        },
      );
...
export const addFileRecordsToGrist = async (
  options: any,
  collectionId: string,
  runRecordId: number,
  dir: string,
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (md5ToRecord.has(md5)) {
      ...
      if (runRecordId !== existingRecord.fields.Generated_by) {
        console.log(
          `Updating existing record ${existingRecord.id} source Run.`,
        );
        ...
        console.warn(
          "TODO: remove irrelevant previous run record : " +
            existingRecord.fields.Generated_by,
        );
      }
      continue;
    }
...
export const getCorrespondingCollectionId = async (
  options: any,
  ...
  const existingCollectionId = collectionRecords.find((r) =>
    r.fields.UUID === collectionUuid
  )?.id;
  if (!existingCollectionId) {
    throw console.error(
      `No existing collection found in Grist with UUID ${collectionUuid}. Please create the collection in Grist first and re-run the script.`,
    );
  }
  return existingCollectionId;
};

export const getCorrespondingRunId = async (
  options: any,
  collectionId: string,
  runName: string,
) => {
  ...
  const existingRunId = runRecords.find((r) =>
    r.fields.name === runName && r.fields.collection === collectionId
  )?.id;
  return existingRunId;
};

export const logScriptEnd = (
  scriptDefinition: ScriptDefinition,
  runRecordId: number,
) => {
  console.log(
    `Run ${scriptDefinition.runName} created in Grist with ID: ${runRecordId}`,
  );
};
...
export const getScriptDefinition = (
  toolName: string,
  type: SCRIPT_TYPE,
  cliRunName: string,
  inputFolder: string,
  ...
  const normalizedOutputFolder = outputFolder
    ? outputFolder.replace(/^\/+/u, "")
    : undefined;
  const scriptDefiniton: ScriptDefinition = {
    ...
    outputFolder: normalizedOutputFolder
      ? join(normalizedOutputFolder, runName)
      : undefined,
  };
  return scriptDefiniton;
};

export const getIdsByMD5FromGrist = async (
  options: any,
  md5Set: Set<string>,
): Promise<number[]> => {
...
export const getRegexFromInput = (
...
export const getFilesMatchingRegex = async (
  repo: string,
  inputRegex: string,
): Promise<string[]> => {
...
export const getGristIdsByMatchedFiles = async (
  options: any,
  matchedFiles: string[],
): Promise<number[]> => {
  const md5Set = new Set(
    await Promise.all(matchedFiles.map(async (p) => getMD5FromFile(p))),
  );
...
export const fetchWithRetry = async (
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (
        (res.status === 504 || res.status === 503) && attempt < maxRetries - 1
      ) {
        const delay = Math.pow(2, attempt) * 5000; // 5s, 10s, 20s
        console.warn(
          `⏳ Server returned ${res.status}, retrying in ${delay}ms... (attempt ${
            attempt + 1
          }/${maxRetries})`,
        );
      ...
      if (e.name === "AbortError") {
        if (attempt < maxRetries - 1) {
          ...
          console.warn(
            `⏳ Request timeout, retrying in ${delay}ms... (attempt ${
              attempt + 1
            }/${maxRetries})`,
          );
          await new Promise((resolve) => setTimeout(resolve, delay));
