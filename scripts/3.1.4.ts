...
import { basename, extname, join, relative } from "jsr:@std/path@0.224.0";
...
const { options } = await new Command()
  .name("SHERLOOK 3.1.4")
  .description(
    "Find CSV files by regex and convert them to SQLite using headers",
  )
...
for (const csvPath of matches) {
  ...
  const tableName =
    basename(csvPath, extname(csvPath)).replace(/[^a-zA-Z0-9_]/g, "_") ||
    "data";
  ...
  const outputName = makeUniqueName(
    `${basename(csvPath, extname(csvPath))}.sqlite`,
  );
  ...
  try {
    ...
    const insertStmt = db.prepareQuery(
      `INSERT INTO "${tableName}" (${
        sanitizedColumns.map((col) => `"${col}"`).join(", ")
      }) VALUES (${placeholders})`,
    );
